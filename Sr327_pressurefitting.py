...
def rho_c(T,Pone,Ptwo,Pthree,alpha,beta,gamma,Offset,save=False,name=''):
    Nx = 24; Ny = 5
    input = 5; output = 6
    mirrorinputpin = "V["+str(Nx-input)+",1]"; mirroroutputpin = "V["+str(Nx-output)+","+str(Ny)+"]"

    df = simulate(5,6,Pone,Ptwo,Pthree,alpha,beta,gamma,Offset,save=save,name=name)[0][0]
...
def fitpressureimplicit(data,pressure,infolist,save=True):
    ...
    guess = [pressure,pressure,pressure,0,0,0,0]
    bounds = [[1,1,1,-3,-3,-3,-10**3],[10**(20),10**(20),10**(20),3,3,3,10**3]]
    ...
    ax.plot(T,rho_c(T,params[0],params[1],params[2],params[3],params[4],params[5],params[6],save=True,name='DiFitData'+str(pressure)+'DAT/'),linewidth=4,ls='-.',label=r'Fitted Simulation')
    RVTaxes(ax)
    if save == True:
...
if __name__ == "__main__":
    ...
    for i in range(7):
        ...
        infolist = np.array(infolist).reshape(1,10)
