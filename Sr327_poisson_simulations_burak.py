...
def newR_c(T,pone=1,ptwo=1,alpha=0,beta=0,gamma=0,offset=0,xone=0,xtwo=0,widthone=1,widthtwo=1):
    ...
    return 1000.0*(f*(1.0+E*pow(T,2+alpha))/pone + F*(pow(T,1+beta))*g*(1.-f)/ptwo + (G+offset+H*pow(T,1+gamma))*(1.-g)*(1.-f))
...
def newry(T,Ny,pone=1,ptwo=1,alpha=1,beta=1,gamma=1,offset=0,xone=0,xtwo=0,wone=1,wtwo=1):
    return 20*newR_c(T,pone,ptwo,alpha,beta,gamma,offset,xone,xtwo,wone,wtwo)/(Ny-1)
...
def simulate(Vin,Vout,P,Ptwo=1,alpha=1,beta=1,gamma=1,offset=0,xone=0,xtwo=0,wone=1,wtwo=1,save=True,verbose = False,name = '',conlyflag=False):
    meshsize = 2
    Nx = 12*meshsize
    Ny = 2*meshsize+1
    ...
    for Tctr in range(0,N):
        ...
        if verbose == True:
            print("T: "+str(T))
        Rx = rx(T,Nx); Ry = newry(T,Ny,P,Ptwo,alpha,beta,gamma,offset,xone,xtwo,wone,wtwo)
